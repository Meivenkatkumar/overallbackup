...
if($type=="admin")
{
  ...
  echo "<strong>".$row['teacher']."</strong> is willing to open course on <strong>".$row['name']."</strong><br>";
  echo htmlspecialchars_decode($data);
  echo " for <strong>class ".$row['class']."</strong><br><button type='submit' name='auth' value='".$row['id']."' class='btn btn-primary'>Allow</button><button type='submit' name='deny' value='".$row['id']."' class='btn btn-danger'>Reject</button>";
  }
  echo "</form>";
}
if($type=="student")
{
 ...
 echo "<form method='post' action='profile.php'>";
  while($row=$rows->fetch_assoc())
  {
    ...
    $data=str_replace("<i<","&lt;i&gt;", $data);
    $data=str_replace(">i>","&lt;/i&gt", $data);
  echo $row['name']." taken by ".$row['teacher'].":";
  echo htmlspecialchars_decode($data);
  echo " for class .".$row['class']."<button type='submit' name='join' value='".$row['id']."' class='btn btn-primary'>Join</button>";
    }
  }
 echo "</form>";
