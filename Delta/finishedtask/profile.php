...
if(isset($_POST['backuptime']))
{
  ...
  $cycletiming=$_POST['backup_cycle'];
  $arguements=explode(":",$time);
  $hour=$arguements[0];
  $minute=$arguements[1];
  $filename='date "+\%Y-\%m-\%d"';
  if($cycletiming=="daily")
    $string="'".$minute." ".$hour." * * * mysqldump -u 'root' -p'Meiven212!' mysqldump --databases project > /var/backups/mysqlbackup/backup_$(".$filename.").sql' > /var/www/html/includes/mysqldump.txt";
  else if($cycletiming=="weekly")
    $string="'".$minute." ".$hour." * * 1 mysqldump -u 'root' -p'Meiven212!' mysqldump --databases project > /var/backups/mysqlbackup/backup_$(".$filename.").sql' > /var/www/html/includes/mysqldump.txt";
  else if($cycletiming=="monthly")
    $string="'".$minute." ".$hour." 1 * * mysqldump -u 'root' -p'Meiven212!' mysqldump --databases project > /var/backups/mysqlbackup/backup_$(".$filename.").sql' > /var/www/html/includes/mysqldump.txt";
  exec("echo ".$string);
}
 ...
 <div class="container-fluid">
  ...
  <div class="navbar navbar-collapse" id="navbarResponsive" style="padding:0px 30px;">
    <ul class="navbar-nav ml-auto">
     ...
     if($type=="admin")
     {
       echo ' <li class="nav-item"><a class="nav-link" href="analysis.php">Web Analysis</a></li>';
     }
     if($type=="teacher")
     {
...
if(isset($_GET['examover']))
{
 ...
 while($row=$rows->fetch_assoc())
 {
  ...
  echo "<div class='imgcontain' style='margin:10px auto;'>
 ...
 while($row=$rows->fetch_assoc()){
  if($row['loginstatus']==1)
  {
    if($row['picture']==null)
    {
      ...
      $picture=$row['picture'];
     echo "<button type='submit' value='".$row['name']."' onclick='chatname(this.value)' class='btn btn-primary btn-block' style='padding-top:20px;'><p class='chatdesc'><img src='".$picture."' class='rounded-circle' alt='pic' height='50' width='50'><bold>".$row['name']." </bold>online</p></button>";
  } 
  else if($row['loginstatus']==0)
  {
    ...
    elseif($lastseen<86400)
     ...
     echo "<button type='submit' value='".$row['name']."' onclick='chatname(this.value)' class='btn btn-primary btn-block' style='padding: auto;padding-top:20px;'><p class='chatdesc'><img src='".$picture."' class='rounded-circle' alt='pic' height='50' width='50'><bold>".$row['name']."</bold> last seen ".$lastseenstring." ago</p></button>";
  }
 }
 if($type=="student")
 ...
 while(mysqli_stmt_fetch($stmt))
 {
   echo "<button type='submit' value='".$groupname."' onclick='chatname(this.value)' class='btn btn-primary btn-block'>".$groupname."</button>";
    ...
    <div class="col-sm" id="columncenter">
      ...
      <input type="time" name="backuptime" class="form-control" style="max-width: 300px;float:none;margin: 10px auto;" required><br>
      <div class="form-check-inline">
      <label><input type="radio" name="backup_cycle" class="form-check-input" value="daily" style="margin:10px 15px;" required>Daily</label>
      <label><input type="radio" name="backup_cycle" class="form-check-input" value="weekly" style="margin:10px 15px;" required>Weekly</label>
      <label><input type="radio" name="backup_cycle" class="form-check-input" value="mothly" style="margin:10px 15px;" required>Monthly</label>
      </div><br><br>
      <button type="submit" class="btn btn-primary">Set Backup</button>
    </form>';
  }
